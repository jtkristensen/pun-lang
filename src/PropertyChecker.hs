...
generateGenerator s@(is, bs) Integer'           =
  oneof $
...
generateGenerator s@(is, bs) Boolean'           =
  oneof $
...
generateGenerator s (Variable' index)  = generateGenerator s (resolve index $ fst s)
-- Todo, generate more interesting things here!
generateGenerator s t@(type1 :*: type2)  =
  do t1 <- generateGenerator s type1
     t2 <- generateGenerator s type2
     return $ Pair t1 t2 t
-- Todo, generate more interesting things here!
generateGenerator (is, bs) (type1 :->: type2) =
  do x  <- generateName
